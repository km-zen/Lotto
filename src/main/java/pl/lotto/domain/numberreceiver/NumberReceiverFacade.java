...
public class NumberReceiverFacade {
    ...
    public NumberReceiverResponseDto inputNumbers(Set<Integer> numbersFromUser) {
        ...
        TicketDto generatedTicket = TicketDto.builder().hash(hash).numbers(numbersFromUser).drawDate(drawDate).build();
        Ticket savedTicket = Ticket.builder().hash(hash).numbers(generatedTicket.numbers()).drawDate(generatedTicket.drawDate()).build();
    ...
    public List<TicketDto> retrieveAllTicketsByNextDrawDate(LocalDateTime date) {
        ...
        return ticketRepository.findAllTicketsByDrawDate(date).stream().filter(ticket -> ticket.drawDate().isEqual(date)).map(ticket -> TicketDto.builder().hash(ticket.hash()).numbers(ticket.numbers()).drawDate(ticket.drawDate()).build()).collect(Collectors.toList());
    }
    ...
    public TicketDto findByHash(String hash) {
        ...
        return TicketDto.builder().hash(ticket.hash()).numbers(ticket.numbers()).drawDate(ticket.drawDate()).build();
    }
